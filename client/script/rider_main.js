...
class Main {
  constructor() {
    this.socket = new WebSocket(
      `ws://localhost:8080/chat?id=${id}&user_type=rider`
    );
    ...
    this.passengers_nearby = [];
    this.nearest_passenger = null;
  ...
  onmessage(event) {
    ...
    if (this.ride_request) {
      ...
      let repeat = false;
      this.passengers_nearby.forEach((i) => {
        console.log(data.id);
        if (this.passengers_nearby[i].id == data.id) repeat = true;
      });

      if (!repeat) {
        this.passengers_nearby.push(data);
        this.update();
      }
    }
  }
  ...
  setMinDistance(min_distance) {
    if (min_distance === false || this.nearest_passenger === null) {
    ...
    document.querySelector(
      ".notification-bar"
    ).innerHTML = `A passenger is ${min_distance} away from you`;

    map.clearPath();
    map.setPath(map.location_self, this.nearest_passenger);
  }
   ...
   */
  setPassengerOnMap(threshold = 4) {
    for (let i = 0; i < this.passengers_nearby.length; i++) {
      map.markUserOnMap(this.passengers_nearby[i], "passenger", true);
    }
  }
  ...
  update(toggle = true) {
    ...
    if (this.update_var === false && toggle == true) {
      map.markUserOnMap(map.location_self, "rider", false, true);
      this.setPassengerOnMap();

      // nearest user location update
      let min_distance;
      if (this.passengers_nearby.length > 0) {
        min_distance = map.calculateDistance(
          map.location_self.lat,
          map.location_self.lng,
          this.passengers_nearby[0].lat,
          this.passengers_nearby[0].lng
        );
        this.nearest_passenger = this.passengers_nearby[0];
      }

      for (let i = 1; i < this.passengers_nearby.length; i++) {
        let distance = map.calculateDistance(
          map.location_self.lat,
          map.location_self.lng,
          this.passengers_nearby[i].lat,
          this.passengers_nearby[i].lng
        );
        if (min_distance > distance) {
          min_distance = distance;
          this.nearest_passenger = this.passengers_nearby[i];
        }
      }
...
let rider = new Main(id);
window["map"] = map;
window.onload = rider.main.bind(rider);
